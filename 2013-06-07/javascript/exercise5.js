...
var complete_village = function(n,m) {
	var x_road = T([0])([vx])(CUBOID([distance,vy+(distance+vy)*(n-1)]));
	var x_roads = STRUCT(REPLICA(m-1)([x_road, T([0])([distance+vx])]));

	var y_road = T([1])([vy])(CUBOID([vx+(distance+vx)*(m-1),distance]));
	var y_roads = STRUCT(REPLICA(n-1)([y_road, T([1])([distance+vy])]));

	return STRUCT([village(n,m), x_roads, y_roads]);
}

var village1 = complete_village(5,6);

var village2 = complete_village(10,6);
